...
namespace ViewWindow.Model
{
    ...
    /// <summary>
    /// 此类用作 HALCON 窗口的包装类
    /// HWindow.HWndCtrl 负责可视化。
	/// 您可以使用 GUI 组件输入或使用鼠标移动和缩放可见图像部件。
    /// HWndCtrl 使用图形堆栈来管理显示的标志性对象。每个对象都链接到图形上下文，从而确定如何绘制对象。
	/// 可以通过调用更改图形设置（）来更改上下文。
	/// 图形"模式"由类 GraphicsContext 定义，并映射 HAnd 中提供的大多数dev_set_* 运算符。
	/// </summary>
	public class HWndCtrl
	{
        ...
        public bool isDrawLine = false;//手头在画线就阻止startX,startY乱更新
        ...
        /* dispROI 是一个标志，用于了解何时将 ROI 模型添加到绘制例程，以及是否响应 ROI 对象的鼠标事件 */
        private int           dispROI;
        ...
        /// <summary> 
        /// 把HALCON 列表中的对象绘制到 HALCON 窗口中。 该列表不应包含超过 MAXNUMOBJLIST 对象，否则第一个条目将从列表中删除。
        /// </summary>
        ...
        private void HMouseWheel(object sender, HMouseEventArgs e)//缩放
        {
            //关闭缩放事件
            if (drawModel)
            {
        ...
        /// <summary>
        /// 设置线的宽度，可以通过HWindowControl的HalconWindow属性修改更多东西，不过现在我用不上
        /// </summary>
        /// <param name="width"></param>
        public void SetLineWidth(int width)
        {
            viewPort.HalconWindow.SetLineWidth(width);
        }
        /********************************************************************/
		...
		/*******************************************************************/
		private void moveImage(double motionX, double motionY)//移动图片
		{
        ...
        //X,Y：当前坐标
        /// <summary>
        /// 1.进入移动图片模式MODE_VIEW_MOVE
        /// 2 如果roi需要创建且还没有被创建，原地创建一个新的roi，并将其添加到 ROI 列表。（2，3在mouseDownAction中实现）
        /// 3 如果 ROI已经存在，被点到后变成激活 ROI 。并在ROIController中设置全局变量，当前点到的roi的id：activeROIidx
        /// 4.设置这条直线activeHandleId，告诉是哪一个点（头中尾三部分）被选中（在mouseDownAction中的distToClosestHandle中实现）
        /// </summary>
        /// <param name="sender"></param>
        /// <param name="e"></param>
        private void mouseDown(object sender, HalconDotNet.HMouseEventArgs e)//鼠标点下
		{
          
                if (drawModel)//缩放模式时鼠标不起作用，滚轮起作用
                {
                    return;
                }

                stateView = MODE_VIEW_MOVE;//移动图片模式
                mousePressed = true;
                int activeROIidx = -1;

                if (roiManager != null && (dispROI == MODE_INCLUDE_ROI))
                {
                    activeROIidx = roiManager.mouseDownAction(e.X, e.Y);//获取点击位置的roi的id，没有就是-1
                }

                if (activeROIidx == -1)//没点到roi，就是扯动图片
                {
                    switch (stateView)
                    {
                        case MODE_VIEW_MOVE:// 一定是这个吧，上面都设置了
                            startX = e.X;//鼠标点下时的位置作为起始位置
                            startY = e.Y;
                            break;
		...
		/*******************************************************************/
		private void mouseUp(object sender, HalconDotNet.HMouseEventArgs e)//鼠标抬起
		{
		...
		/*******************************************************************/
		private void mouseMoved(object sender, HalconDotNet.HMouseEventArgs e)//鼠标移动
		{
            //关闭缩放事件
            if (drawModel)
            {
			...
			if (!mousePressed)//鼠标没有按下，那么鼠标随便动，没影响
				return;

            ////鼠标按下了，并在移动
            if (roiManager != null && (roiManager.activeROIidx != -1) && (dispROI == MODE_INCLUDE_ROI))
			{
				roiManager.mouseMoveAction(e.X, e.Y);//移动roi，之前要先设定好激活的roi的部分的id：activeHandleIdx
            }
			else if (stateView == MODE_VIEW_MOVE)//移动图片
			{
	 
                    motionX = ((e.X - startX));//当前位置减去初始位置，等于已经鼠标移动的距离
                    motionY = ((e.Y - startY));
            

				if (((int)motionX != 0) || ((int)motionY != 0))//鼠标动了
				{
                    if(!isDrawLine)
                    {
                        moveImage(motionX, motionY);//移动图片
                        startX = e.X - motionX;//移动后的位置设为起始位置
                        startY = e.Y - motionY;
                    }
			...
			else if (stateView == MODE_VIEW_ZOOMWINDOW)//缩放图片
			{
